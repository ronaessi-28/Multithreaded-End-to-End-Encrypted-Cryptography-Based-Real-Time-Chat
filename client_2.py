...
# ... (gcd, mod_inverse, is_prime remain the same) ...
def gcd(a, b):
...
def generate_rsa_keys(): 
    """
    Generates a simple RSA public-private key pair for Client 2.
    Uses fixed primes p_c2=59, q_c2=47.
    """
    p_c2 = 59 
    q_c2 = 47

    if not (is_prime(p_c2) and is_prime(q_c2)):
...
def decrypt_message(private_key, base64_encoded_string):
    ...
    try:
        ...
        print(f"[CLIENT C2] Decryption/Decoding error: {e}")
...
class ChatClient:
    def __init__(self, master_root):
        ...
        self.root.title("Encrypted Chat Client 2") 
    ...
    def connect_to_server(self):
        try:
            ...
            if response.startswith("ACK::"):
                if self.login_frame.winfo_exists(): self.login_frame.destroy()
                self.setup_chat_ui()
    ...
    def reset_connection_ui(self):
        ...
        if self.root.winfo_exists():
            self.root.title("Encrypted Chat Client 2 (Disconnected)")
    ...
    def send_message_event(self, event=None):
        ...
        if message_text and self.connected:
            ...
            try:
                ...
                print(f"[CLIENT C2] Encryption/Send error: {e}")
    ...
    def process_message_buffer(self):
        ...
        if self.receive_buffer.startswith("KEY::"):
            try:
                ...
                print(f"[CLIENT C2] Partial or malformed KEY message: {self.receive_buffer[:100]}, Error: {e}")
                return False 
            except Exception as e:
                print(f"[CLIENT C2] Exception during KEY message processing: {e}, buffer: {self.receive_buffer[:100]}")
                return False
        ...
        # RELAY::sender_name::base64_payload
            ...
            try:
                ...
                print(f"[CLIENT C2] Error parsing/decrypting RELAY message: {e}, data: {self.receive_buffer[:100]}")
    ...
    def receive_messages(self):
        while self.connected:
            try:
                ...
                print(f"[CLIENT C2] Unexpected error in receive_messages: {e}")
                self.connected = False 
                self.reset_connection_ui_from_thread()
                break
        ...
        if self.client_socket:
            try:
                ...
                pass
            self.client_socket = None
        print("[CLIENT C2] Receive thread terminated.")
